...
func encodeEmptyFlagArray(v reflect.Value) ([]byte, error) {
	...
	buffer.Write(Uint32ToBytes(uint32(l)))
...
func encodeValue(v reflect.Value) (bytes []byte, err error) {
	...
	case reflect.Int16:
		...
		if vv, ok := v.Interface().(int16); ok {
			bytes = append(bytes, Uint16ToBytes(uint16(vv))...)
			break
		}
		vvv := reflect.ValueOf(v.Interface()).Uint()
		bytes = append(bytes, Uint16ToBytes(uint16(vvv))...)
	case reflect.Int32:
		//vv := uint32(v.Interface().(int32))
		//bytes = append(bytes, Uint32ToBytes(vv)...)
		if vv, ok := v.Interface().(int32); ok {
			bytes = append(bytes, Uint32ToBytes(uint32(vv))...)
			break
		}
		vvv := reflect.ValueOf(v.Interface()).Uint()
		bytes = append(bytes, Uint32ToBytes(uint32(vvv))...)
	case reflect.Int64:
		//vv := uint64(v.Interface().(int64))
		//bytes = append(bytes, Uint64ToBytes(vv)...)
		if vv, ok := v.Interface().(int64); ok {
			bytes = append(bytes, Uint64ToBytes(uint64(vv))...)
			break
		}
		vvv := reflect.ValueOf(v.Interface()).Uint()
		bytes = append(bytes, Uint64ToBytes(uint64(vvv))...)
	case reflect.Uint8:
	...
	case reflect.Float32:
		if vv, ok := v.Interface().(float32); ok {
			bytes = append(bytes, Float32ToBytes(vv)...)
			break
		}
		vvv := reflect.ValueOf(v.Interface()).Float()
		bytes = append(bytes, Float32ToBytes(float32(vvv))...)
	case reflect.Float64:
		if vv, ok := v.Interface().(float64); ok {
			bytes = append(bytes, Float64ToBytes(vv)...)
			break
		}
		vvv := reflect.ValueOf(v.Interface()).Float()
		bytes = append(bytes, Float64ToBytes(vvv)...)
	case reflect.Slice:
		// FIXME 还有别的类型可能会报错
		switch vv := v.Interface().(type) {
		case []uint8:
		...
		case []string:
			...
			for i := 0; i < l; i++ {
				bytes = append(bytes, StringToBytes(vv[i])...)
			}
		case []int: // int32
			l := len(vv)
			bytes = append(bytes, Uint32ToBytes(uint32(l))...)
			for i := 0; i < l; i++ {
				bytes = append(bytes, Uint32ToBytes(uint32(vv[i]))...)
			}
		case []uint: // uint32
			l := len(vv)
			bytes = append(bytes, Uint32ToBytes(uint32(l))...)
...
func decodeValue(f reflect.Value, bytes []byte) []byte {
	...
	case reflect.Uint64:
		...
		bytes = bytes[8:]
	case reflect.Float32:
		f.SetFloat(float64(BytesToFloat32(bytes[:4])))
		bytes = bytes[4:]
	case reflect.Float64:
		f.SetFloat(BytesToFloat64(bytes[:8]))
		bytes = bytes[8:]
	case reflect.Slice:
		...
		case reflect.Int, reflect.Uint, reflect.Int32, reflect.Uint32:
			il := int(BytesToUint32(bytes[:4]))
			bytes = bytes[4:]
			slice := reflect.MakeSlice(f.Type(), l, l)
			for ii := 0; ii < il; ii++ {
				sliceValue := reflect.New(slice.Type().Elem()).Elem()
				i := BytesToUint32(bytes)
				bytes = bytes[4:]
